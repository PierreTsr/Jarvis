...
RSpec.describe Business, type: :model do
  ...
  it "should filter by price" do
    expect(Business.new.get_business_from_category('hardware', "10025").with_prices("$").any? { |business| business['name'] == 'United Hardware' }).to be_truthy
    expect(Business.new.get_business_from_category('hardware', "10025").with_prices("$").any? { |business| business['name'] == 'Sugar Hill Hardware' }).to be_falsy
  end
